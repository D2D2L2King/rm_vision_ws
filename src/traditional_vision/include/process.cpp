...
cv::Mat image_processing(const cv::Mat& image) {
    ...
    cv::inRange(hsv, lower_blue, upper_blue, mask_blue); // 根据蓝色范围创建掩码
    // cv::inRange(hsv, lower_red, upper_red, mask_red); // 根据红色范围创建掩码
    cv::imshow("mask_blue", mask_blue); // 显示蓝色掩码
    // cv::imshow("mask_red", mask_red); // 显示红色掩
